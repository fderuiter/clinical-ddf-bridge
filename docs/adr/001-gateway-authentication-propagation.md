# ADR 001: Centralized API Gateway Authentication and Header Propagation
...
## Consequences
### Positive
*   **Decoupled Microservices:** Downstream services remain lightweight and decoupled from the identity provider and OIDC complexities.
*   **Performance:** Cryptographic verification of JWTs is done once at the edge. Downstream services only perform a fast HMAC signature check, satisfying low-latency constraints (< 15ms).
*   **Security:** Centralized edge protection prevents unauthorized access. The HMAC signature prevents spoofing of identity headers even if an attacker manages to route directly to a downstream service.

### Negative
*   **Shared Secret Management:** Both the gateway and all downstream services must securely share and manage the `GATEWAY_SECRET`.
*   **Tight Coupling on Header Format:** Any changes to the header propagation format (`X-User-Id`, `X-User-Roles`) must be synchronized across the gateway and all consuming microservices.
