...
def requires_signature(artifact_type: str, metadata_json: Optional[Dict[str, Any]] = None) -> bool:
    """
    ...
    """
    if metadata_json and (metadata_json.get("requires_signature") is True or metadata_json.get("require_signature") is True):
        return True
...
def extract_signature_from_content(content: str) -> Tuple[Optional[str], Optional[bytes], Optional[str]]:
    """
    ...
    if "-----BEGIN CERTIFICATE-----" in content:
        cert_match = re.search(r"(-----BEGIN CERTIFICATE-----.*?-----END CERTIFICATE-----)", content, re.DOTALL)
        sig_match = re.search(r"-----BEGIN SIGNATURE-----\s*(.*?)\s*-----END SIGNATURE-----", content, re.DOTALL)
        ...
        if cert_match:
            ...
            signed_data = content
            signed_data = re.sub(r"-----BEGIN CERTIFICATE-----.*?-----END CERTIFICATE-----", "", signed_data, flags=re.DOTALL)
            signed_data = re.sub(r"-----BEGIN SIGNATURE-----.*?-----END SIGNATURE-----", "", signed_data, flags=re.DOTALL)
            return cert_pem, sig_bytes, signed_data.strip()

    # 2. Try XML-style tags
    if "<Signature" in content or "<X509Certificate" in content:
        cert_match = re.search(r"<X509Certificate>\s*(.*?)\s*</X509Certificate>", content, re.DOTALL)
        sig_match = re.search(r"<SignatureValue>\s*(.*?)\s*</SignatureValue>", content, re.DOTALL)
        ...
        if cert_match:
            ...
            signed_data = content
            signed_data = re.sub(r"<Signature\b[^>]*>.*?</Signature>", "", signed_data, flags=re.DOTALL)
            signed_data = re.sub(r"<X509Certificate>.*?</X509Certificate>", "", signed_data, flags=re.DOTALL)
            signed_data = re.sub(r"<SignatureValue>.*?</SignatureValue>", "", signed_data, flags=re.DOTALL)
            return cert_pem, sig_bytes, signed_data.strip()

    return None, None, None


def verify_x509_signature(cert_pem: str, signature_bytes: bytes, signed_data: bytes) -> bool:
    """
...
def validate_document_signature(
    ...
    if not cert_pem or not sig_bytes:
        if is_required:
            return False, f"Missing required digital signature for artifact type '{artifact_type}'."
        else:
