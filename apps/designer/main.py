...
from apps.designer.soa_models import (
    ...
    UpdateProcedureActivityRequest,
    SoACell,
...
# ==========================================
# Schedule of Activities (SoA) API Endpoints
# ==========================================

@app.get(
    "/api/v1/studies/{study_id}/versions/{version_id}/soa",
    response_model=SoAMatrixProjectionResponse,
)
async def get_soa_matrix(study_id: str, version_id: str) -> SoAMatrixProjectionResponse:
    """
    Retrieves the complete Schedule of Activities (SoA) matrix projection for a given study and version.
    """
    # Sample static timing windows
    w_baseline = TimingWindow(
        id="win_baseline",
        ...
        window_forward=0,
        time_unit="DAYS",
        audit=SoAAuditMetadata(created_by="usr_system")
    )
    w_week2 = TimingWindow(
        id="win_week2",
        name="Week 2 Follow-up Window",
        target_day=14,
        window_back=2,
        window_forward=2,
        time_unit="DAYS",
    ...
    # Sample static epoch, arms, visits, procedures
    arm_active = StudyArm(
        id="arm_active",
        name="Active Arm",
        arm_type="TREATMENT",
    ...
    visit_v2 = Visit(
        ...
        timing_window_id="win_week2",
        audit=SoAAuditMetadata(created_by="usr_system")
    )

    proc_blood = ProcedureActivity(
        id="proc_blood",
        name="Blood Draw",
        code="271649006",
        audit=SoAAuditMetadata(created_by="usr_system")
    )
    proc_ecg = ProcedureActivity(
        id="proc_ecg",
    ...
    # Core cells showing applicability and conditional timing logic
    cell_1 = SoACell(
        arm_id="arm_active",
        epoch_id="ep_treatment",
        visit_id="v_baseline",
        procedure_id="proc_blood",
        is_applicable=True,
    ...
    cell_2 = SoACell(
        ...
        epoch_id="ep_treatment",
        visit_id="v_week2",
        procedure_id="proc_ecg",
        is_applicable=True,
        is_conditional=True,
        conditional_reason="Perform only if subject reports cardiovascular discomfort."
    )
    cell_3 = SoACell(
        arm_id="arm_placebo",
        epoch_id="ep_treatment",
    ...
    return SoAMatrixProjectionResponse(
        study_id=study_id,
        study_version_id=version_id,
        arms=[arm_active, arm_placebo],
        epochs=[epoch_treatment],
        visits=[visit_v1, visit_v2],
        procedures=[proc_blood, proc_ecg],
        timing_windows=[w_baseline, w_week2],
...
@app.post(
    ...
    response_model=StudyArm,
    status_code=201,
)
async def create_study_arm(
    study_id: str, version_id: str, payload: CreateStudyArmRequest
) -> StudyArm:
    """Creates a new study arm for a specific study version."""
    return StudyArm(
        id="arm_created_123",
        name=payload.name,
        arm_type=payload.arm_type,
        type_concept_id=payload.type_concept_id,
        description=payload.description,
        audit=SoAAuditMetadata(
            created_by="usr_9921a88b2c410",
            reason_for_change=payload.change_reason
        )
    )
...
@app.post(
    ...
    response_model=Epoch,
    status_code=201,
)
async def create_epoch(
    study_id: str, version_id: str, payload: CreateEpochRequest
) -> Epoch:
    """Creates a new epoch for a specific study version."""
    return Epoch(
        id="ep_created_123",
        name=payload.name,
...
async def create_visit(
    study_id: str, version_id: str, payload: CreateVisitRequest
) -> Visit:
    """Creates a new visit for a specific study version."""
    return Visit(
        id="v_created_123",
        name=payload.name,
        visit_window_days=payload.visit_window_days,
...
@app.post(
    ...
    response_model=ProcedureActivity,
    status_code=201,
)
async def create_procedure_activity(
    study_id: str, version_id: str, payload: CreateProcedureActivityRequest
) -> ProcedureActivity:
    """Creates a new procedure or activity for a specific study version."""
...
@app.post(
    ...
    response_model=TimingWindow,
    status_code=201,
)
async def create_timing_window(
    study_id: str, version_id: str, payload: CreateTimingWindowRequest
) -> TimingWindow:
    """Creates a new timing window definition for a specific study version."""
    return TimingWindow(
        id="win_created_123",
        name=payload.name,
        target_day=payload.target_day,
        window_back=payload.window_back,
        window_forward=payload.window_forward,
