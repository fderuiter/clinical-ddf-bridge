...
async def create_library_object_version(driver, object_id: str, new_properties: Dict[str, Any]):
    """
    ...
    async with driver.session() as session:
        ...
        if exists:
            ...
            result = await session.run(create_query, object_id=object_id, props=new_properties)
            
        record = await result.single()
        return record["new_props"]

async def update_study_properties(driver, study_id: str, user_id: str, change_reason: str, properties: Dict[str, Any]):
    """
...
async def get_study_differences(driver, study_id: str, action_id1: str, action_id2: str) -> List[Dict[str, Any]]:
    """
    ...
    """
    async with driver.session() as session:
        result = await session.run(query, study_id=study_id, action_id1=action_id1, action_id2=action_id2)
        record = await result.single()
        if not record:
            return []
