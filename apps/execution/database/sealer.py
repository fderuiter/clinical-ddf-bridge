...
async def execute_audit_sealing_cycle(db: AsyncSession, limit: int = 100) -> Optional[str]:
    """
    ...
    for rec in records:
        timestamp_str = rec.timestamp.isoformat() if hasattr(rec.timestamp, "isoformat") else str(rec.timestamp)
...
async def start_background_sealer(session_maker: Any, interval: Optional[float] = None) -> None:
    """
    ...
    async def sealer_loop():
        logger.info("Background ledger sealer started with interval %s seconds.", interval)
        while _should_run:
            try:
                async with session_maker() as db:
                    block_hash = await execute_audit_sealing_cycle(db)
                    if block_hash:
                        logger.info("Successfully sealed block with hash: %s", block_hash)
            except Exception as e:
