        ...
        async with session.begin():
            ...
            try:
                ...
                if 'protocol' not in payload:
                    raise ValueError("Validation Failed: 'protocol' missing from study definition.")

                # Proceed to translation
                # Requirement 2: CDISC ODM schemas
                odm_root = ET.Element("ODM", xmlns="http://www.cdisc.org/ns/odm/v1.3", FileOID=f"ODM.{study_id}")
                study = ET.SubElement(odm_root, "Study", OID=f"Study.{study_id}")
                ET.SubElement(study, "GlobalVariables").text = "Cadence Generated Study"
                meta_odm = ET.SubElement(study, "MetaDataVersion", OID="MDV.1", Name="Version 1.0")
                ...
                for item in items:
                    ...
                    # Add ODM Definition
                    ET.SubElement(meta_odm, "ItemDef", OID=item_id, Name=item_name, DataType=item_type)

                    # Add OpenRosa XForm Elements
                    ET.SubElement(data, item_id)
                    ET.SubElement(model, "xf:bind", nodeset=f"/{item_id}", type=f"{item_type}")
                ...
                # Format outputs
                odm_str = minidom.parseString(ET.tostring(odm_root)).toprettyxml(indent="  ")
                openrosa_str = minidom.parseString(ET.tostring(openrosa_root)).toprettyxml(indent="  ")
