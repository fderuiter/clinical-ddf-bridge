...
def test_clean_token():
    ...
    assert vm.clean_token(".github/workflows/production-pipeline.yml") == ".github/workflows/production-pipeline.yml"
    assert vm.clean_token("tests/.") == "tests/"
    assert vm.clean_token("(e.g., `docs/adr/2026-06-06-usdm-pydantic-models.md`).") == "e.g., `docs/adr/2026-06-06-usdm-pydantic-models.md"
...
def test_is_potential_path_ref():
    ...
    assert not vm.is_potential_path_ref("/openapi.json", root_dirs, root_files)  # not in root files
    ...
    assert vm.is_potential_path_ref("/docs/LOCAL_DEV_ENVIRONMENT.md", root_dirs, root_files)
...
def test_resolve_path(tmp_path):
    ...
    res = vm.resolve_path("apps/execution/main.py", md_file, repo_root, root_dirs, root_files)
    assert res == repo_root / "apps/execution/main.py"

    # 2. Workspace root-relative path starting with leading slash
    res = vm.resolve_path("/docs/LOCAL_DEV_ENVIRONMENT.md", md_file, repo_root, root_dirs, root_files)
    ...
    assert vm.resolve_path("https://google.com", md_file, repo_root, root_dirs, root_files) is None
    assert vm.resolve_path("your-placeholder-file", md_file, repo_root, root_dirs, root_files) is None
...
def test_validate_path(tmp_path):
    ...
    assert "Referenced path './nonexistent.md' does not exist." in vm.errors[0]["message"]
...
def test_validate_cli_command_python_and_pytest(tmp_path):
    ...
    vm.validate_cli_command(["pytest", "tests/test_main.py"], 12, md_file, repo_root, root_dirs, root_files)
    assert len(vm.errors) == 0

    # Invalid pytest target
    vm.validate_cli_command(["pytest", "tests/test_nonexistent.py"], 15, md_file, repo_root, root_dirs, root_files)
    assert len(vm.errors) == 1
    assert "Target path 'tests/test_nonexistent.py' for executable 'pytest' does not exist." in vm.errors[0]["message"]
...
def test_validate_cli_command_flag_checks(tmp_path):
    ...
    # Valid flags
    vm.validate_cli_command(["pytest", "-v", "--cov=apps"], 5, md_file, repo_root, root_dirs, root_files)
    assert len(vm.errors) == 0

    # Malformed flag
    vm.validate_cli_command(["pytest", "---cov=apps"], 10, md_file, repo_root, root_dirs, root_files)
    assert len(vm.errors) == 1
    assert "Malformed or invalid CLI flag structure: '---cov=apps'" in vm.errors[0]["message"]
...
def test_validate_docker_compose_scenarios(tmp_path):
    ...
    vm.validate_cli_command(
        ...
        20, md_file, repo_root, root_dirs, root_files
    )
    assert len(vm.errors) == 1
    assert "Docker compose file 'docker/docker-compose.yml' does not exist." in vm.errors[0]["message"]
...
"""
    ...
    assert "Target path 'tests/test_nonexistent.py' for executable 'pytest' does not exist." in error_msgs
