...
def test_study_differences_missing_version(client):
    ...
    with patch("httpx.AsyncClient.get", return_value=mock_response):
        ...
        assert "Target version v2 is missing from the registry" in response.json()["detail"]
...
def test_study_differences_registry_404(client):
    ...
    with patch("httpx.AsyncClient.get", side_effect=httpx.HTTPStatusError("404", request=mock_request, response=mock_response)):
        response = client.get(
            "/api/v1/studies/study-123/differences?action_id1=v1&action_id2=v2",
            headers=get_auth_headers()
        )
...
def test_study_differences_registry_error(client):
    ...
    with patch("httpx.AsyncClient.get", side_effect=httpx.HTTPStatusError("500", request=mock_request, response=mock_response)):
        response = client.get(
            "/api/v1/studies/study-123/differences?action_id1=v1&action_id2=v2",
            headers=get_auth_headers()
        )
