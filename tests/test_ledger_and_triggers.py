...
from sqlalchemy import text, select, String
from sqlalchemy.orm import Mapped, mapped_column

from apps.execution.database.core import db_manager
from apps.execution.database.context import current_session, current_user_id, current_change_reason
from apps.execution.database.models import Base, AuditLog, AuditLedgerSeal, AuditedModel
...
async def test_prevent_audit_log_mutation():
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            with pytest.raises(Exception, match="Modification or deletion of audit logs is strictly prohibited"):
                await session.execute(text("UPDATE audit_logs SET user_id = 'hacker' WHERE record_id = 'rec_1';"))
                
            with pytest.raises(Exception, match="Modification or deletion of audit logs is strictly prohibited"):
                await session.execute(text("DELETE FROM audit_logs WHERE record_id = 'rec_1';"))
...
async def test_prevent_audit_ledger_seals_mutation():
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            with pytest.raises(Exception, match="Modification or deletion of audit logs is strictly prohibited"):
                await session.execute(text("UPDATE audit_ledger_seals SET current_block_hash = 'tampered';"))
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            with pytest.raises(Exception, match="Hard deletions are strictly forbidden"):
                await session.execute(text("DELETE FROM audited_clinical_records WHERE id = 'rec_100';"))
...
async def test_out_of_band_update_triggers_audit_entry():
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            await session.execute(text("UPDATE audited_clinical_records SET data_value = 'tampered' WHERE id = 'rec_200';"))
            await session.commit()
            
    # Verify that the DB trigger captured the out-of-band change and inserted an AuditLog record
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            res = await session.execute(select(AuditLog).where(AuditLog.table_name == "audited_clinical_records"))
            logs = res.scalars().all()
            update_logs = [l for l in logs if l.action == "UPDATE"]
            
            assert len(update_logs) == 1
            assert update_logs[0].new_values["data_value"] == "tampered"
            assert update_logs[0].user_id in ("system", "system_process")  # default out-of-band value
            assert update_logs[0].change_reason in ("system_operation", "Automated system operation")  # default out-of-band value
...
async def test_ledger_sealing_and_validation():
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            log1 = AuditLog(id="log_1", table_name="tb", record_id="r1", action="INSERT", user_id="u1", change_reason="r")
            log2 = AuditLog(id="log_2", table_name="tb", record_id="r2", action="UPDATE", user_id="u2", change_reason="r")
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            res = await session.execute(select(AuditLog).where(AuditLog.id.in_(["log_1", "log_2"])))
            logs = res.scalars().all()
            assert len(logs) == 2
            for log in logs:
    ...
    async with db_manager.get_session_maker()() as session:
        async with session.begin():
            await session.execute(text("DROP TRIGGER trg_lock_audit_trail_logs_update;"))
            await session.execute(text("UPDATE audit_logs SET change_reason = 'tampered' WHERE id = 'log_1';"))
            await session.commit()
