...
class Artifact(BaseModel):
    ...
    code: str = Field(..., description="Stable, unique code for the artifact (e.g. '01.01.01')")
...
class Section(BaseModel):
    ...
    zone_code: int = Field(..., description="Stable code of the parent zone")
    artifacts: List[Artifact] = Field(default_factory=list, description="List of approved artifacts in this section")
...
class Zone(BaseModel):
    ...
    code: int = Field(..., description="Stable, unique integer code for the zone (1 to 11)")
    name: str = Field(..., description="Canonical display name of the zone")
    sections: List[Section] = Field(default_factory=list, description="List of sections in this zone")
...
class TaxonomyCatalog(BaseModel):
    ...
    version: str = Field(..., description="Unique version name of the catalog (e.g. 'v3.2.0')")
    zones: List[Zone] = Field(..., description="The 11 canonical zones included in this catalog version")
